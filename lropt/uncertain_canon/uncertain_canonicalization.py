...
class Uncertain_Canonicalization(Reduction):
    ...
    def remove_uncertainty(self, unc_lst, uvar, std_lst, num_constr):
        ...
        if num_unc_fns > 0:
            ...
            if has_isolated == 1:
                for idx in range(num_constr):
                    aux_constraint += [z_cons + supp_cons[idx] + z_new_cons[idx] == -z_unc[idx]]
            else:
                aux_constraint += [z_cons + supp_cons[0] == -z_unc[0]]
            new_expr, new_constraint, lmbda = uvar.conjugate(z_unc, supp_cons, num_constr, k_ind=0)
            aux_expr = aux_expr + new_expr
    ...
    def remove_uncertainty_mro(self, unc_lst, uvar, std_lst, num_constr):
        ...
        if num_unc_fns > 0:
            ...
            for k_ind in range(uvar.uncertainty_set._K):
                ...
                if has_isolated == 1:
                    for idx in range(num_constr):
                        aux_constraint += [z_cons + supp_cons[k_ind][idx]+ z_new_cons[idx] == -z_unc[k_ind][idx]]
                else:
            ...
            aux_constraint = []
            for k_ind in range(uvar.uncertainty_set._K):
                aux_expr, new_constraint, lmbda, sval = uvar.conjugate(u_shape, 0, num_constr, k_ind)
                cur_expr = aux_expr
                for expr in std_lst:
                    cur_expr = cur_expr + expr
                aux_constraint = aux_constraint + new_constraint + [cur_expr <= 0]
