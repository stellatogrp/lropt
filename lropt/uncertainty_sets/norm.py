        ...
        if not isinstance(var, Variable):
            ...
            if self.data is not None or self.paramT is not None:
                if shape == 1:
                    ...
                    return self.rho * lmbda + self._d@supp_newvar - newvar@self.paramb, constr, lmbda
                else:
                    constr = []
                    lmbda = Variable(shape)
                    ...
                    return self.rho * lmbda + supp_newvar@self._d - newvar@self.paramb, constr, lmbda
            else:
                if shape == 1:
                    lmbda = Variable()
                    supp_newvar = Variable(len(self._d))
                    ...
                    constr += [self._c.T@supp_newvar == supp_var[0]]
                    constr += [supp_newvar >=0]
                    return self.rho * lmbda + self._d@supp_newvar- var[0]@self.paramb, constr, lmbda
                else:
                    constr = []
                    lmbda = Variable(shape)
                    constr += [lmbda >= 0]
                    ...
                    for ind in range(shape):
                        ...
                        constr += [self._c.T@supp_newvar[ind] == supp_var[ind]]
                    return self.rho * lmbda + supp_newvar@self._d - var@self.paramb, constr, lmbda
