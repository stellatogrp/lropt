...
class TestMultiStage(unittest.TestCase):
    def test_multistage(self):
        ...
        policy = trainer.create_cvxpylayer(parameters = all_params, reduced_variables = [uall])
        ...
        paramvals, x_base1, u_base1, \
            train_vals1, train_cons1 = trainer.multistage_train(simulator, 
                                ...
                                optimizer = "SGD",lr= lr, momentum = 0, init_alpha = 0.0, scheduler = True, lr_step_size = 20, lr_gamma = 0.5, contextual = True, test_batch_size = test_batch_size,x_endind = x_endind, init_weight=init_weights,init_lam = 0.001, init_mu = 0.001, mu_multiplier = 1.01)