...
class TestMultiStage(unittest.TestCase):
    def test_multistage(self):
        ...
        K = 4
        c = torch.tensor([(1 + 0.5 * np.sin(np.pi *
                                             (t - 1) / (T * 0.5)))
                                             for t in range(1, T + 1)])*0.1
        ...
        d_star = np.array([1000 * (1 + 0.5 *
                                   np.sin(np.pi * (t - 1) /
                                          (T * 0.5))) for t in range(1, T + 1)])
        ...
        demand_dist = torch.distributions.MultivariateNormal(
            torch.tensor(d_star),torch.tensor(cov))
        ...
        def plot_inventory_levels(ax, low_values,
                                  mid_values, high_values,random_values,rho):
            ax.plot(range(0, T + 1), low_values,
                    label='Disturbance Lower Bound',
                    color='blue', linestyle='--')
            ax.plot(range(0, T + 1), mid_values,
                    label='Nominal Disturbance', color='green', linestyle='-')
            ax.plot(range(0, T + 1), high_values,
                    label='Disturbance Upper Bound',
                      color='red', linestyle='--')
            ax.plot(range(0, T + 1), random_values,
                     label='Random Trajectory', color='orange', linestyle='--')
        ...
        def baseline_problem(x_init, cval, pval, hval):
            d = lropt.UncertainParameter(T,
                            uncertainty_set = lropt.Ellipsoidal(p=2,
                                                rho=init_eps,c = lhs,
                                                d = rhs, a = cov, b = d_star))
            # d = d_star
            q = cp.Variable(T)
            y = cp.Variable(T)
            u = cp.Variable(T)
            ...
            constraints = [0 <= q,
                        q <= Qmax]
            for time in range(T):
                constraints +=[Vmin <= x_init + cp.sum(q[:time+1])
                               - cp.sum(d[:(time+1)]) ]
                constraints +=[x_init + cp.sum(q[:(time+1)])
                               - cp.sum(d[:(time+1)])<= Vmax]
                constraints += [y[time] >= hval[time]*x_init
                                + hval[time]*cp.sum(q[:(time+1)])
                                - hval[time]*cp.sum(d[:(time+1)]) ]
                constraints += [y[time] >= -pval[time]*x_init
            ...
            prob.solve()
            return prob.objective.value, w.value, q.value,\
                  y.value, u.value, z.value

        _, _, qval1, _,_, \
            _ = baseline_problem(x_init = 100,
        ...
        def baseline_problem_aro(init_val, cval, pval, hval):
            d = lropt.UncertainParameter(T,uncertainty_set =
                                          lropt.Ellipsoidal(p=2,rho=init_eps,
                                            c = lhs, d = rhs, a = cov,
                                            b = d_star))
            # d = d_star
            q = cp.Variable(T)
            ...
            objective = C
            constraints = [cval@(q + q_var@d) + cp.sum(y + y_var@d)
                           + cp.sum(u + u_var@d) + cp.sum(z) <= C,
                        0 <= q+ q_var@d,
                        q+q_var@d <= Qmax]
            ...
            for time in range(T):
                constraints +=[Vmin <= x_init +
                               cp.sum((q+q_var@d)[:time+1]) -
                                 cp.sum(d[:(time+1)]) ]
                constraints +=[x_init + cp.sum((q+q_var@d)[:(time+1)])
                               - cp.sum(d[:(time+1)])<= Vmax]
                constraints += [(y + y_var@d)[time] >= hval[time]*x_init +
                                 hval[time]*cp.sum((q+q_var@d)[:(time+1)])
                                 - hval[time]*cp.sum(d[:(time+1)]) ]
                constraints += [(y + y_var@d)[time] >= -pval[time]*x_init
        ...
        baseline_prob, x_init, w_baseline,q,q_var,y,y_var,\
            u,u_var,z = baseline_problem_aro(init_val = 100,
                                cval=c[:T].detach().numpy(),
                                pval=p[:T].detach().numpy(),
                                  hval = h[:T].detach().numpy())
        baseline_prob.solve()
        ...
        for i in range(1, T + 1):
            low = (cp.sum((qval+qmat@rhs_upper)[:i]) -
                   cp.sum(rhs_upper[:i]) + x_init).value
            mid = (cp.sum((qval+qmat@d_star)[:i]) -
                   cp.sum(d_star[:i]) + x_init).value
            high = (cp.sum((qval-qmat@rhs_lower)[:i]) -
                     cp.sum(-rhs_lower[:i]) + x_init).value
            traj = (cp.sum((qval+qmat@(random_traj.detach().numpy()))[:i]) -
                     cp.sum(random_traj.detach().numpy()[:i]) + x_init).value
            low_values.append(low)
            mid_values.append(mid)
            high_values.append(high)
        ...
        d = lropt.UncertainParameter(K,uncertainty_set =
                    lropt.Ellipsoidal(p=2,rho=1,c = lhs_new,
                                      d = rhs_new, data = np.zeros((1,K))))
        # d = lropt.UncertainParameter(K,uncertainty_set =
        ...
        all_params = [x_hat, d_star_val, q_hat, d_hat, y_hat, u_hat,
                      p_xhat, h_xhat, w, t, e_ind, p_ind, h_ind,c_ind,Ymat_ref]
        ...
        for i in range(K):
            constraints += [Vmin <= x_hat[0] + cp.sum(q_hat) +
                             cp.sum([e_ind[j] for j in range(i+1)])@q +
                             Qmat_new[i]@d - cp.sum(d_hat) -
                             cp.sum([e_indvar[j] for j in range(i+1)])@d]

            constraints += [cp.sum([e_ind[j] for
                                    j in range(i+1)])@Qmat==Qmat_new[i]]

            constraints += [x_hat[0] + cp.sum(q_hat)
                            + cp.sum([e_ind[j] for j in range(i+1)])@q +
                            Qmat_new[i]@d - cp.sum(d_hat) -
                            cp.sum([e_indvar[j] for j in range(i+1)])@d <= Vmax]

            constraints += [h_ind[i]@Qmat == Hmat[i]]
            constraints += [p_ind[i]@Qmat == Pmat[i]]
            constraints += [y[i] + Ymat_new[i]@d >= h_xhat[i]
                            + h_ind[i]@q + Hmat[i]@d - h_indvar[i]@d ]

            constraints += [y[i] + Ymat_new[i]@d>= -p_xhat[i]
                             - p_ind[i]@q - Pmat[i]@d +  p_indvar[i]@d ]
        ...
        policy = trainer.create_cvxpylayer(parameters =
                                           all_params, variables = [uall])

        class InvSimulator(lropt.Simulator):
            def simulate(self, x, u):
                u = u[0]
                x_hat,d_star_val, q_hat, d_hat, y_hat, u_hat,\
                _, _, _, tval, _, _, _,_,_ = x
                t = int(tval[0])
                ...
                p_new = torch.vstack([p[t+j+1]*np.sum([e_new[i] for
                        i in range(j+1)],axis=0) for j
                        in range(K)]).repeat(batch_size,1,1)
                h_new = torch.vstack([h[t+j+1]*np.sum([e_new[i] for
                            i in range(j+1)],axis=0) for j
                            in range(K)]).repeat(batch_size,1,1)
                c_new = (c[t:t+K]*np.sum([e_new[i] for
                            i in range(K)],axis=0)).repeat((batch_size,1))
                e_new = torch.tensor(e_new).repeat(batch_size,1,1)
                Ymat_ref = torch.tensor(Y_matref[:,t:t+K]).repeat(batch_size,1,1)
                x = [x_hat,d_star_val, q_hat, d_hat, y_hat, u_hat,
                     p_x,h_x,w_new,t_new,e_new,p_new,h_new,c_new,Ymat_ref]
                # print(x)
                return x

            def stage_cost_eval(self,x,u):
                u = u[0]
                x_hat, _,q_hat, _, _, u_hat, \
                _, _, _, t, _,_, _,_,_ = x
                assert x_hat.shape[0] == u.shape[0]
                t = int(t[0])
                x_hat = x_hat[:,t]
            ...
            def init_state(self,batch_size, seed=None):
                ...
                e_new = eye_concat[t:t+K,t:t+K]
                p_new = torch.vstack([p[t+j]*np.sum([e_new[i] for i
                        in range(j+1)],axis=0) for j
                        in range(K)]).repeat(batch_size,1,1)
                h_new = torch.vstack([h[t+j]*np.sum([e_new[i]
                        for i in range(j+1)],axis=0) for j
                        in range(K)]).repeat(batch_size,1,1)
                c_new = (c[t:t+K]*np.sum([e_new[i]
                    for i in range(K)],axis=0)).repeat((batch_size,1))
                e_new = torch.tensor(e_new).repeat(batch_size,1,1)
        ...
        trainer_settings.set(simulator=simulator, multistage=True, policy=policy, time_horizon=T,
                            num_iter=epochs, batch_size=batch_size, init_rho=1.5, seed=0,
                            init_A=init_a, init_b=init_b, optimizer="SGD", lr=lr, momentum=0,
                            init_alpha=0.0, scheduler=True, lr_step_size=20, lr_gamma=0.5,
                            contextual=True, test_batch_size=test_batch_size, x_endind=x_endind,
                            init_weight=init_weights, init_lam=0.001, init_mu=0.001,
                            mu_multiplier=1.01)
